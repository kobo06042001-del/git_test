...
st.title("smile: 😄")
st.title("ghost: 👻")
st.caption("캡션을 한번 넣어 봤음")
# st.markdown("마크다운 문법도 지원합니다. **굵게**, *기울임*, ~~취소선~~")
st.markdown("abcdefghijklmnop. **굵게**, *기울임*, ~~취소선~~")
...
# 메트릭
st.metric(label="tesla", value="$180", delta="$2")
st.metric(label="온도", value="25 ℃", delta="1.5 ℃")  # ㄹ + 한자 => ℃

# 컬럼으로 영역 나누어 표기
col1, col2, col3, col4 = st.columns(4)
col1.metric(label="속도", value="120km/h", delta="5km/h")
col2.metric(label="환율(USD)", value="1,471 원", delta="+30 원")
col3.metric(label="환율(EUR)", value="1,590 원", delta="+20 원")
col4.metric(label="엔(JPY)", value="1,050 원", delta="+5 원")
...
st.write(f"당신은 선택한 색깔은 : red[{favorite_color}], 입니다.")
...
name = st.text_input(
    ...
    placeholder="예: 뮌헨, 비엔나, 상트페테르부르크"
)
st.write(f"당신이 가고 싶은 여행지는 :green[{name}] 입니다.")
...
st.write(f"당신이 좋아하는숫자는 :green[{number}]입니다.")
